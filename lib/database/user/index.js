...
module.exports = {
  ...
  requestLanguage: function* (session) {
    const languages = []
    const list = [ 'فارسی', 'русский', 'English', 'العربية', 'Español', 'Italiano']

    list.forEach((text, key) =>
      languages.push([ { text, callback_data: 'localization_' + key } ])
    )

    yield telegram.sendMessage(session, 'Please select your language to continue.',
