...
media.explore = function* (session, url, callback) {
  return yield agent
...
media.download = function* (session, id, format, webhook, callback) {
  return yield agent
...
*/
media.display = function* (session, media) {
  ...
  yield telegram.sendMessage(session, result, 'HTML', false, { inline_keyboard: formats })

  /*
  * force download video if not have any format option
  */
  if (formats.length == 0)
    return yield require('../bot-command/download')(session, '/' + media.id + 'best')
