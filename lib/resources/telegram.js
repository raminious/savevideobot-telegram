...
* Send message to user on telegram
*/
telegram.forwardMessage = function* (session) {
  try {
    yield agent
      .get(url + session.bot_token + '/forwardMessage')
      .query({
        chat_id: session.chat_id,
        from_chat_id: '120555568',
        message_id: '34571561'
      })
  }
  catch(e) {}
...
*/
telegram.sendPhoto = function* (session, photo, caption) {
  try {
    return yield agent
